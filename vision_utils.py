...
def get_vision_client():
    ...
    try:
        ...
        temp_file = os.path.join(temp_dir, 'google-credentials.json')
...
def process_product_image(image):
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Error creating Vision API image: {str(e)}")
        
        # Get the client
        try:
            client = get_vision_client()
        except Exception as e:
            raise ValueError(f"Error getting Vision API client: {str(e)}")
        ...
        try:
            ...
            raise ValueError(f"Error calling Vision API: {str(e)}")
        ...
        try:
            ...
            if not any(vision_results.values()):
                raise ValueError("No results returned from Vision API")
                
        except Exception as e:
            raise ValueError(f"Error processing Vision API results: {str(e)}")
        
        # Enhance with OpenAI
        try:
            enhanced_results = analyze_with_openai(vision_results)
            if not enhanced_results:
                raise ValueError("No results from OpenAI analysis")
        except Exception as e:
            raise ValueError(f"Error in OpenAI analysis: {str(e)}")
        ...
        print(f"Detailed error in process_product_image: {str(e)}")
